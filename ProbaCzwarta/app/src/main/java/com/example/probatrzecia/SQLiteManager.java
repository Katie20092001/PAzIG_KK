        ...
        try (Cursor result = sqLiteDatabase.rawQuery("SELECT * FROM " + TABLE_NAME_MEDICINES, null)) {
            if (result.getCount() != 0) {
                while (result.moveToNext()) {
                    ...
                    String medicineName = result.getString(2);
                    String medicineDescription = result.getString(3);
                    Date date = parseDate(result.getString(4));
                    Time time = parseTime(result.getString(5));

                    Medicine medicine = new Medicine(id_medicine, id_pet, medicineName, medicineDescription, date, time);
